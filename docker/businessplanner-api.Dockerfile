# docker/businessplanner-api.Dockerfile
# Monolith businessplanner-api binary — all domains compiled into one.
...
# runtime image can run `businessplanner-api migrate` without bundling all
...
COPY --from=build /build/businessplanner-api ./businessplanner-api
COPY services/drogon-host/config ./config

# Migration SQL files (needed by `businessplanner-api migrate`)
...
ENTRYPOINT ["./businessplanner-api"]
